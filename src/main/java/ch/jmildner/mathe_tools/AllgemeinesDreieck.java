...
public final class AllgemeinesDreieck {
	...
	// aenderung 28.12.2020
	private static boolean DEBUG = true;

	private static String f14 = "%14.8f  ";
	private static String s14 = "%14s  ";

	private static String dFormat = f14 + f14 + f14 + f14 + f14 + f14 + f14 + f14 + f14 + f14 + f14;

	private static String kFormat = s14 + s14 + s14 + s14 + s14 + s14 + s14 + s14 + s14 + s14 + s14;
	...
	private void checkWinkel() throws Exception {
		...
		if (!TrigonometricTools.equals(ws, 180.0)) {
			...
			throw new Exception("Winkelsumme ist nicht 180 [" + w1 + "+" + w2 + " + " + w3 + " = " + ws + "]");
		}
	}
	...
	public static void outKopf(String typ) {
		...
		String kopf = String.format(kFormat, "Seite_1", "Seite_2", "Seite_3", "Hoehe_1", "Hoehe_2", "Hoehe_3",
				"Winkel_1", "Winkel_2", "Winkel_3", "WinkelSumme", "Flaeche");
	...
	public AllgemeinesDreieck(String typ, double x, double y, double z) throws Exception {
		switch (typ) {
		case "SSS":
		...
		if (TrigonometricTools.equals(A1s, A2s) && TrigonometricTools.equals(A2s, A3s)
				&& TrigonometricTools.equals(A3s, A1h) && TrigonometricTools.equals(A1h, A2h)
				&& TrigonometricTools.equals(A2h, A3h)) {
			flaeche = A1s;
		} else {
			throw new Exception
			// System.out.println
			("die Flaechen stimmen nicht ueberein [" + A1s + ",  " + A2s + ",  " + A3s + ",  " + A1h + ",  " + A2h
					+ ",  " + A3h + "]");
		}
	}
	...
	public void outDaten() {
		String zeile = String.format(dFormat, s1, s2, s3, h1, h2, h3, w1, w2, w3, wSumme, flaeche);

		out(zeile);
	}

	public double seiteCosinussatz(double s1, double w3, double s2) {
		return Math.sqrt(qu(s1) + qu(s2) - (-2 * s1 * s2 * TrigonometricTools.getCosFromDegrees(w3)));
	}

	private double seiteSinussatz(double s1, double w1, double w2) {
		return s1 * TrigonometricTools.getSinFromDegrees(w2) / TrigonometricTools.getSinFromDegrees(w1);
	}
	...
	public String toString() {
		return "AllgemeinesDreieck [s1=" + s1 + ", s2=" + s2 + ", s3=" + s3 + ", h1=" + h1 + ", h2=" + h2 + ", h3=" + h3
				+ ", w1=" + w1 + ", w2=" + w2 + ", w3=" + w3 + "]";
	}
	...
	private double winkelSinussatz(double s1, double w1, double s2) {
		return TrigonometricTools.getDegreesFromSin(TrigonometricTools.getSinFromDegrees(w1) * s2 / s1);
	}
}
